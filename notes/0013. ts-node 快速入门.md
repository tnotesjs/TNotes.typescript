---
id: 757ad566-147e-4619-a205-303fd6acebda
description: 记录了 ts-node 的作用、原理、安装与使用方法，并对其与 tsc、node 的关系及实际开发中的应用场景进行了详细说明。
---
...
```json {8-10} [package.json 准备测试脚本 scripts]
{
  "name": "1",
  "version": "1.0.0",
  "description": "",
  ...
  "scripts": {
    "test": "echo \"Error: no test specified\" && exit 1",
    "ts-node": "ts-node",
    "ts-node-showConfig": "ts-node --showConfig",
...
console.log(`原字符串: ${input}`)
console.log(`转换后: ${output}`)
```
