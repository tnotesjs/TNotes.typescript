...
- 主要是配合联合类型缩小类型用的。
- 「定义变量阶段（放大）」和「使用变量阶段（缩小）」
  - 在定义变量的时候，我们使用联合类型，相当于将多个类型合并，表示这个变量可以是这些类型中的任意一个。
  - 在使用变量的时候，我们需要明确变量具体的类型，这时候就需要使用「类型收窄（Type Narrowing）」和「类型守卫（Type Guards）」来配合联合类型进行“类型缩小”。
- 关键点总结：
