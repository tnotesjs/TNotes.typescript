...
- [8. 🤔 any 的类型推断都有什么问题？](#8--any-的类型推断都有什么问题)
  - [8.1. noImplicitAny](#81-noimplicitany)
  - [8.2. 特殊情况：](#82-特殊情况)
- [9. 🤔 noImplicitAny 配置的作用是？](#9--noimplicitany-配置的作用是)
- [10. 🤔 「any 类型的污染问题」是指？](#10--any-类型的污染问题是指)
- [11. 🤔 空数组会被 TS 推断为什么类型？](#11--空数组会被-ts-推断为什么类型)
...
- 在非 strict 模式下，TS 会关闭对 any 的类型检查，是指你可以把 any 类型视作任意类型来使用，比如当做函数 a() 当做对象 a.xxx 都不会触发报错。
...
- 对于开发者没有指定类型、TypeScript 必须自己推断类型的那些变量，如果无法推断出类型，TypeScript 就会认为该变量的类型是 any。
  - 注意：TS 的具体推断行为会受到 TS 配置文件的影响，如果开启了 noImplicitAny 配置，那么在 TS 推断出 any 类型时就会报错。
- 可视化地看到那些被推断为 any 类型的成员
  - 在一些 IDE 中（比如在线的 TS Playground），如果变量被隐式地推断为 any 类型，底部会有 3 个点。
  - ![图 0](https://cdn.jsdelivr.net/gh/tnotesjs/imgs@main/2025-10-19-08-23-59.png)
- 开发建议：
  - 对于那些类型不明显的变量，一定要显式声明类型，防止被推断为 any；
  - 开启 TS 的 noImplicitAny 配置；
  - 使用 let 和 var 声明变量时，如果不赋值，就一定要显式声明类型，否则可能存在安全隐患；
- 提示：
  - 本节示例中的程序可以丢到 TS Playground 中快速验证。
...
- `tsc --noImplicitAny 1.ts`
  - 该命令使用了 noImplicitAny 编译选项进行编译，这时 1.ts 中的函数 add() 就会报错。
...
虽然 x1、x2 被 TS 推断为了 any 类型，并且我们也打开了 noImplicitAny 配置，但是上述程序并不会报错。

这是一种特殊情况，使用 var、let 声明的变量如果被推断为 any 类型是不会报错的，无论是否开启了 noImplicitAny 配置。

但是对于 const 声明的变量并不会存在该问题，因为 const 声明的变量必须在声明的同时进行初始化，TS 不会将其隐士地推断为 any 类型。
...
3. `result.toUpperCase()`

2 执行之后，result 被 add 污染了，也成为了 any 类型，因此，上述写法 3 并不会报错。
...
toUpperCase 是字符串特有的，数字类型的原型链上默认是不存在这样一个 API 的，及时报错，才是我们更希望看到的。
